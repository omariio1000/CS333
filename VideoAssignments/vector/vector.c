...
int main(int argc, char *argv[]) {
    ...
    NOISY_DEBUG_PRINT;
    if (is_verbose) fprintf(stderr, "input file processes\n");
    output_data(ofile, vector, num_elements);
    NOISY_DEBUG_PRINT;
    if (is_verbose) fprintf(stderr, "sorting byt ascending value\n");
...
point_t *read_data(FILE *ifile, int *num_elements) {
    ...
    if (is_verbose) fprintf(stderr, "%d points read form input\n", ni);
    *num_elements = ni;
    NOISY_DEBUG_PRINT;
    return vector;
}
...
void output_data(FILE *ofile, point_t *vector, int num_elements) {
    ...
    if (is_verbose) fprintf(stderr, "%d points in output\n", num_elements);
    NOISY_DEBUG_PRINT;
}
